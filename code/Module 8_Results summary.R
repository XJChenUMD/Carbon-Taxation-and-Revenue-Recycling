...
A <- as.data.frame(Cover_decile_agg[,,6])
A$Group <- rownames(Cover_decile_agg[,,6])
A %>% pivot_longer(-Group,
                   names_to = "Region",
                   values_to = Name_SP[6]) ->X5


Cover_All <- merge(merge(merge(merge(X1,X2),X3),X4),X5)
rm(A,X1,X2,X3,X4,X5,Name_SP)
#---------------
...
#Fig S6: national results under different tax + SA
#===================
for (i in 1:dim(Tax_Scenarios)[3]) {
  for (j in 1:dim(SP_Scenarios)[3]) {
    ...
    A <- as.data.frame(OUT_TAXSP)
    A$PovertyChg <- A$PPop_ipl_tax-A$PPop_ipl_ori
    A$TAX <- dimnames(Tax_Scenarios)[[3]][i]
    A$SA <- dimnames(SP_Scenarios)[[3]][j]
    A$Nation <- rownames(A)
    
    if(i == 1 & j == 1) B <- A else B <- rbind(B,A)
  }
}

B <- B[-which(B$SA %in% c("PMT","Null")),]
B <- B[-which(B$TAX %in% "Null"),]

Nation_best <-  as.data.frame(array(NA,dim = c(length(Reg_Inclu),3))) 
colnames(Nation_best) <- c("nation","SA_best","Poverty effect")

for (r in 1:length(Reg_Inclu)) {
  C <- B[which(B$Nation %in% Reg_corr$WBGDPreg[r]),]
  a <- rowsum(C$PovertyChg,C$SA)
  
  Nation_best[r,1] <- Reg_corr$WBGDPreg[r]
  Nation_best[r,3] <- min(a)
  if (sum(min(a)==a) == 1)  Nation_best[r,2] <- rownames(a)[which(a %in% min(a))]
  if (sum(min(a)==a) == 2) Nation_best[r,2] <- str_c(rownames(a)[which(a %in% min(a))][1],"/",rownames(a)[which(a %in% min(a))][2])
  if (sum(min(a)==a) == 3) Nation_best[r,2] <- str_c(rownames(a)[which(a %in% min(a))][1],"/",rownames(a)[which(a %in% min(a))][2],
                                                     "/",rownames(a)[which(a %in% min(a))][3])
  if (sum(min(a)==a) == 4) Nation_best[r,2] <- "No difference"
}

write.csv(Nation_best,
          file = str_c(pathout,"/Nation_best SA.csv"))
rm(Nation_best,A,B,C,a)
#===================


save(Nation_best,CBDR,Luxury,Revenue,RevenuetoGDP,Cost_perEXP_All,Tax_reg_C,Tax_reg_P,Elasticity_raw,
...
rm(CBDR,Luxury,Revenue,RevenuetoGDP,Cost_perEXP_All,
   ...
   q,r,a,i,m,n,p,Coun_Sec_Tax,Footprint_InvGov_detail,Global_PovInq_SATAX_100tax,Global_PovInq_SATAX_25tax,
   Global_PovInq_SATAX_30,Global_PovInq_SATAX_75tax,InvGov_detail,Best_Tax,Best_SA,
